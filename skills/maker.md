...
- **우측 하단 고정 위치**에 페이지 표시: `N / Total` (아래 페이지 번호 규칙 참조)
...
**기본 배경 패턴: 흰색 + 주요색 블러 1개**

배경은 흰색(또는 plan.md에 지정된 밝은 배경색) 바탕에, 주요색을 `filter: blur()`로 은은하게 깔아 깊이감을 준다.

**배경은 모든 슬라이드에서 완전히 동일해야 한다.** 블러 위치·크기·투명도를 슬라이드마다 다르게 하지 않는다. 하나의 배경 CSS를 정의하고 모든 슬라이드에 그대로 복사한다.

```css
/* 배경 블러 효과 — 주요색 원형 1개만 사용 */
.bg-blur {
  position: absolute;
...
/* 예시: 우상단에 배치 */
.bg-blur { top: -150px; right: -100px; }
```

- 블러 원형은 **정확히 1개만** 사용한다. 여러 개 배치하면 배경이 얼룩덜룩해진다
- 배경이 흰색일 때 블러 불투명도는 **0.05~0.10** 범위가 적절
- 블러 요소는 반드시 `z-index: 0`, 콘텐츠는 `position: relative; z-index: 1` 이상
- **모든 슬라이드에서 동일한 배경 CSS를 사용한다** (위치, 크기, 투명도 모두 동일)
...
슬라이드에 시각적 풍부함을 더하는 장식 요소. **빈 `<div>`로 구현**한다.

**절대 금지**:
- 이모지(😊🔥✅❌💡 등) 또는 유니코드 아이콘 문자를 장식 목적으로 사용하지 않는다
- **`::before`, `::after`로 색상 띠/바/라인을 만들지 않는다.** 카드 상단·좌측·하단에 컬러 바를 추가하면 디자인 품질이 떨어진다. 장식이 필요하면 빈 `<div>`에 클래스를 부여하여 구현한다
...
페이지 번호는 **모든 슬라이드에서 동일한 위치·동일한 스타일**이어야 한다. 아래 CSS를 그대로 사용한다:
...
- 위치: **우측 하단** (`bottom: 36px; right: 48px`) — 슬라이드마다 위치를 바꾸지 않는다
- 스타일: 14px, 400 굵기, 낮은 불투명도로 콘텐츠를 방해하지 않게
- 형식: `N / Total` (예: `1 / 6`)
- **커버·클로징 포함 모든 슬라이드에 동일하게 적용**한다
